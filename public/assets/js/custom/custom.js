(function () {
	...
	try {
		...
		document.getElementById('sweetalert_2').addEventListener('click', function (e) {
			Swal.fire("عنوان اینجاست!", "...و این متن است!");
		});
		...
		document.getElementById('sweetalert_9').addEventListener('click', function (e) {
			Swal.fire({
				...
				icon: "success",
				buttonsStyling: false,
				confirmButtonText: "<i class='la la-headphones'></i> من بازی هستم!",
	...
	try {
		...
		if (getHeaderssBurgerMenuId) {
			...
			function updateCalendar(date) {
				...
				monthYear.textContent = date.toLocaleDateString('fa-IR', options);
...
try {
	...
	function setTheme(themeName) {
		localStorage.setItem('farol_ltr', themeName);
